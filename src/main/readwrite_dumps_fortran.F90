...
subroutine write_fulldump_fortran(t,dumpfile,ntotal,iorder,sphNG)
 use dim,   only:maxp,maxvxyzu,maxalpha,ndivcurlv,ndivcurlB,maxgrav,gravity,use_dust,&
                 ...
                 ind_timesteps,mhd_nonideal,use_krome,h2chemistry,update_muGamma,maxBxyz
 ...
 use part,  only:xyzh,xyzh_label,vxyzu,vxyzu_label,Bevol,Bevol_label,Bxyz,Bxyz_label,npart,maxtypes, &
                 npartoftypetot,update_npartoftypetot,fxyzu,dBevol, &
 ...
 do ipass=1,2
    ...
    do k=1,ndatatypes
       ...
       if (mhd) then
          ...
          call write_array(4,Bxyz,Bxyz_label,maxBxyz,npart,k,ipass,idump,nums,ierrs(1)) ! Bx,By,Bz
          call write_array(4,fxyzu,(/'fx','fy','fz','fu'/),4,npart,k,ipass,idump,nums,ierr)
          call write_array(4,dBevol,(/'dbx','dby','dbz'/),3,npart,k,ipass,idump,nums,ierr)
          !do j=1,npart
          !  print*, bevol(:,j)
          !enddo
          call write_array(4,Bevol,Bevol_label,4,npart,k,ipass,idump,nums,ierr)
...
subroutine write_smalldump_fortran(t,dumpfile)
 use dim,        only:maxp,maxtypes,use_dust,lightcurve,use_dustgrowth,h2chemistry,maxBxyz
 use io,         only:idump,iprint,real4,id,master,error,warning,nprocs
 use part,       only:xyzh,xyzh_label,npart,Bxyz,Bxyz_label,Bevol,Bevol_label,&
...
!
 ...
 do ipass=1,2
    ...
    if (mhd) then
       ilen(4) = npart
       do k=1,ndatatypes
          call write_array(4,Bxyz,Bxyz_label,maxBxyz,npart,k,ipass,idump,nums,ierr,singleprec=.true.)
          call write_array(4,Bevol,Bevol_label,4,npart,k,ipass,idump,nums,ierr,singleprec=.true.)
       enddo
    endif
...
subroutine read_phantom_arrays(i1,i2,noffset,narraylengths,nums,npartread,npartoftype,&
 ...
 use dim,        only:use_dust,h2chemistry,maxalpha,maxp,gravity,maxgrav,maxvxyzu,do_nucleation, &
                      ...
                      ind_timesteps,use_krome,maxBxyz
 use part,       only:xyzh,xyzh_label,vxyzu,vxyzu_label,dustfrac,dustfrac_label,abundance,abundance_label, &
                      ...
                      Bevol,Bxyz,Bxyz_label,Bevol_label,fxyzu,dBevol,nabundances,iphase,idust, &
 ...
 logical               :: got_sink_data(nsinkproperties),got_sink_vels(3),got_Bxyz(maxBxyz),got_fxyzu(4),got_dbevol(4),got_bevol(4)
    ...
    do k=1,ndatatypes
       ...
       do i=1,nums(k,iarr)
          ...
          case(4)
             ...
             call read_array(fxyzu,(/'fx','fy','fz','fu'/),got_fxyzu,ik,i1,i2,noffset,idisk1,tag,match,ierr)
             call read_array(dBevol,(/'dbx','dby','dbz'/),got_dbevol,ik,i1,i2,noffset,idisk1,tag,match,ierr)
