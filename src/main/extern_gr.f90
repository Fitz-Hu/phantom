...
module extern_gr
!
! Compute terms related to derivatives of the metric which appear
...
!+
!---------------------------------------------------------------
subroutine get_grforce(ipart,xyzhi,metrici,metricderivsi,veli,densi,ui,pi,fexti,dtf)
 ...
 call forcegr(ipart,xyzhi(1:3),metrici,metricderivsi,veli,densi,ui,pi,fexti,ierr)
 if (ierr > 0) then
...
!---------------------------------------------------------------
!+
!  Wrapper of the above, computing accelerations due to metric
...
!+
!----------------------------------------------------------------
pure subroutine forcegr(ipart,x,metrici,metricderivsi,v,dens,u,p,fterm,ierr)
...
!----------------------------------------------------------------
!+
!  compute stress energy tensor on all particles
!+
!----------------------------------------------------------------
subroutine get_tmunu_all(npart,xyzh,metrics,vxyzu,metricderivs,dens,tmunus)
 ...
 do i=1,npart
    if (.not.isdead_or_accreted(xyzh(4,i))) then
       pi = get_pressure(ieos,xyzh(:,i),dens(i),vxyzu(:,i))
       call get_tmunu(xyzh(:,i),metrics(:,:,:,i),vxyzu(1:3,i),&
                      dens(i),vxyzu(4,i),pi,tmunus(:,:,i))
    endif
 enddo
 !$omp end parallel do
...
!-------------------------------------------------------------------------
!+
!  calculate the covariant form of the stress energy tensor
...
!-------------------------------------------------------------------------
!+
!  the following two routines are for testing purposes
!  and could be deleted at some stage (as used in Magnall et al. 2023)
!+
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
!+
!  see above: for testing purposes and could be deleted at some stage
!  (as used in Magnall et al. 2023)
!+
!-------------------------------------------------------------------------
